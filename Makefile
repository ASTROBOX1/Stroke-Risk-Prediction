...
info: ## Show project information
	...
	@echo "$(BLUE)═══════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(YELLOW)Version:$(NC) 2.0.0 (Python 3.14 Compatible)"
	@echo "$(YELLOW)Python:$(NC) $$(python --version 2>&1) $(GREEN)✓$(NC)"
	@echo "$(YELLOW)Location:$(NC) $$(pwd)"
	@echo ""
	@echo "$(YELLOW)Key Libraries:$(NC)"
	@python -c "import pandas as pd, numpy as np, streamlit as st, fastapi; print(f'  Pandas: {pd.__version__}'); print(f'  NumPy: {np.__version__}'); print(f'  Streamlit: {st.__version__}'); print(f'  FastAPI: {fastapi.__version__}')" 2>/dev/null || echo "  $(YELLOW)Run 'make install' first$(NC)"
	@echo ""
